...
export default async function handler(req: any, res: any) {
  ...
  if (!providerName) {
    if (sanitizedModel) {
      if (sanitizedModel.includes('openrouter') || sanitizedModel.startsWith('qwen/') || sanitizedModel.startsWith('deepseek/') || sanitizedModel.includes('google/gemma-3') || sanitizedModel.includes('google/gemini')) {
        ...
        providerName = 'NVIDIA';
      } else if (sanitizedModel.includes('Llama-3.3') || sanitizedModel.includes('DeepSeek-R1') || sanitizedModel.includes('Mistral-7B') || sanitizedModel.startsWith('google/gemma-2') || sanitizedModel.includes('HuggingFace')) {
...
`;
  ...
  try {
    // Build systemPrompt and userPrompt, then immediately use them in the fetch call
    let systemPrompt = '';
    if (isBossChat) {
...
2. Recommend the exact AI model (e.g. DeepSeek-R1 for reasoning, Llama-3.3-70B for general tasks, Qwen-2.5-Coder for code generation, Nemotron for instruction following) that fits the user's code.
