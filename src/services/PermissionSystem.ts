...
export class PermissionSystem {
  ...
  setCurrentUser(userId: string): boolean {
    ...
    console.log(`[Permissions] Current user set to: ${user.name} (${user.role})`);
  ...
  hasPermission(permission: Permission): boolean {
    ...
    this.log(this.currentUser.id, 'PERMISSION_CHECK', 'permission', {
      permission,
      result: status
  ...
  updateUserRole(targetUserId: string, newRole: UserRole): boolean {
    ...
    this.log(
      ...
      'ROLE_UPDATED',
      'user',
      { userId: targetUserId, oldRole, newRole }
    );

    console.log(`[Permissions] User role updated: ${user.name} (${oldRole} → ${newRole})`);
    return true;
  }
  ...
  deactivateUser(userId: string): boolean {
    ...
    user.isActive = false;
    this.log(this.currentUser?.id || 'unknown', 'USER_DEACTIVATED', 'user', { userId });
