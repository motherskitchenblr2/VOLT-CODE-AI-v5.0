...
export class PerformanceOptimizer {
  public static generateReport(code: string): OptimizerReport {
    ...
    const namedImportsMatch = code.match(
      /import\s*\{\s*([\w\s,]+)\s*\}\s*from/g,
    );
      ...
      for (const block of namedImportsMatch) {
        ...
        if (inner) {
          ...
          for (const name of names) {
            ...
            const matches =
              code.match(new RegExp(`\\b${escaped}\\b`, "g")) || [];
            if (matches.length <= 1) {
    ...
    if (code.includes("import { AdminCenter }")) {
      report.lazyLoadingOpportunities.push(
        "AdminCenter (heavy admin cockpit, recommended React.lazy)",
      );
    }
    if (code.includes("import { GitHubWorkspace }")) {
      report.lazyLoadingOpportunities.push(
        "GitHubWorkspace (large file tree explorer, recommended React.lazy)",
      );
    }
    ...
    if (
      ...
      report.memoryLeakAlerts.push(
        "EventListener added without explicit teardown.",
      );
    }
    if (code.includes("setInterval") && !code.includes("clearInterval")) {
      report.memoryLeakAlerts.push(
        "Active interval loop declared without clearInterval cleanup.",
      );
    }

    // Duplicate dependencies
    const reactImports = (
      code.match(/import\s+.*\s+from\s+['"]react['"]/g) || []
    ).length;
    if (reactImports > 1) {
      report.duplicateDependencies.push(
        `Multiple React import paths (${reactImports} headers found). Consolidate imports.`,
      );
    }
