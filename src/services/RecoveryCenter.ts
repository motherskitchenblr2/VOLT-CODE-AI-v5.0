...
export class RecoveryCenter {
  ...
  public async undoLastRepair(checkpointId: string): Promise<string> {
    this.logCallback(
      `[RECOVERY] Intercepting undo command for checkpoint: ${checkpointId}`,
      "info",
    );
    ...
    if (!rawData) {
      ...
      throw new Error(
        `Checkpoint ${checkpointId} not registered inside system recovery registers.`,
      );
    }
    ...
    try {
      ...
      throw new Error(
        `Checkpoint ${checkpointId} data could not be parsed from local cache.`,
      );
    }
    this.logCallback(
      `[RECOVERY] Reverted workspace changes matching checkpoint ${checkpointId}.`,
      "success",
    );
    return content;
  }
  ...
  public async cleanSessionHistory(): Promise<boolean> {
    ...
    this.logCallback(
      "[RECOVERY] Transient session registry cleared.",
      "success",
    );
    return true;
  }
