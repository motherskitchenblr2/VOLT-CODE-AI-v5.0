...
export const TerminalPanel: React.FC<TerminalPanelProps> = ({
  ...
  const handleCommandSubmit = (e: React.FormEvent) => {
    ...
    if (commandLower === "ls" || commandLower === "dir") {
      ...
      outputLines = [
        `Command '${command}' not recognized. Try 'ls', 'whoami', 'git status', 'volt analyze', or 'volt fix'.`,
      ];
    }
      ...
      <div className="h-10 bg-[#121212] border-b border-[#FF5F00]/25 px-4 flex justify-between items-center select-none shrink-0">
        <div className="flex items-center gap-6">
          ...
          <div className="flex gap-1.5">
            {(["diagnostic", "powershell", "ubuntu", "cmd"] as const).map(
              (tab) => (
                <button
                  key={tab}
        ...
        <div className="flex items-center gap-3">
          <button
            onClick={() => {
              if (activeTab === "diagnostic")
                setLogs(["[SYSTEM] Diagnostic log cleared."]);
              else if (activeTab === "powershell")
                setPowershellLogs(["PS C:\\Workspace\\Volt-Code-AI> "]);
              else if (activeTab === "ubuntu")
                setUbuntuLogs(["volt-user@ubuntu:~/workspace$ "]);
              else if (activeTab === "cmd")
                setCmdLogs(["C:\\Workspace\\Volt-Code-AI> "]);
            }}
          ...
          <button
            className="hover:text-[#FF5F00] text-white/50 transition-colors cursor-pointer"
            title="Minimize"
          >
        ...
        {activeTab === "diagnostic" ? (
          ...
          <>
            ...
            <form
              ...
              className="flex items-center text-white mt-0.5"
            >
