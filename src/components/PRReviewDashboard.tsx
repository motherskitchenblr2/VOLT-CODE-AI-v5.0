...
export const PRReviewDashboard: React.FC<PRReviewDashboardProps> = ({
  ...
  const [filterStatus, setFilterStatus] = useState<
    "all" | "open" | "merged" | "squashed" | "ignored"
  >("open");
      ...
      <div className="px-6 py-4 border-b border-white/10 bg-black/50 backdrop-blur-lg">
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="grid grid-cols-4 gap-3">
            ...
            <div className="text-center p-2 rounded-lg bg-white/5">
              <p className="text-xs text-white/60">
                {language === "en" ? "Squashed" : "स्क्वैश"}
              </p>
              <p className="text-lg font-bold text-purple-400">
                {prs.filter((p) => p.status === "squashed").length}
              </p>
            </div>
            <div className="text-center p-2 rounded-lg bg-white/5">
              <p className="text-xs text-white/60">
                {language === "en" ? "Ignored" : "अनदेखा"}
              </p>
              <p className="text-lg font-bold text-red-400">
                {prs.filter((p) => p.status === "ignored").length}
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="flex gap-2">
          {(["all", "open", "merged", "squashed", "ignored"] as const).map(
            (status) => (
              <button
                key={status}
            ...
            filteredPrs.map((pr, idx) => (
              ...
              >
                ...
                <div className="flex items-start justify-between gap-3">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-2">
                      <div
                        className={`p-1.5 rounded border ${getStatusBadgeColor(pr.status)}`}
                      >
                  ...
                  <span
                    className={`px-2.5 py-1 rounded-lg text-xs font-semibold whitespace-nowrap border ${getStatusBadgeColor(pr.status)}`}
                  >
                    {language === "en"
                ...
                <div className="flex items-center gap-2 text-xs text-white/60 mt-2">
                  <span>
                    Branch:{" "}
                    <code className="font-mono text-white/70">{pr.branch}</code>
                  </span>
                  <span>•</span>
                      ...
                      {pr.userDecision && (
                        <div
                          className={`p-3 rounded-lg border ${
                            pr.userDecision === "merge"
                              ? "bg-green-900/20 border-green-500/30"
                              : pr.userDecision === "squash"
