...
const APP_ORIGIN =
  process.env.APP_ORIGIN || 'https://volt-code-ai-v5-0-next-gen-ops-projects.vercel.app';
...
export async function getSessionUsername(req: Req): Promise<string | null> {
  ...
  try {
    await connectToDatabase();
    const session = await AuthSessionModel.findOne({ tokenHash: hashToken(token) }).lean();
...
export function sessionCookieHeader(token: string, opts: { maxAgeMs?: number } = {}): string {
 ...
 */
export async function requireAuth(req: Req & { locals?: { username: string } }, res: Res): Promise<boolean> {
  const username = await getSessionUsername(req);
  if (!username) {
    res.status(401).json({ error: 'Unauthorized', details: 'A valid session is required. Please sign in.' });
    return false;
  }
  req.locals = { username };
  return true;
...
export async function audit(
  ...
  try {
    await connectToDatabase();
    await AuditLogModel.create({ username, action, details: details.slice(0, 1000), status });
  } catch {
...
export async function userHasPassword(username: string): Promise<boolean> {
  try {
    await connectToDatabase();
    const found = await UserSettingsModel.findOne({ username }).select('passwordHash').lean();
    return Boolean(found && typeof found.passwordHash === 'string' && found.passwordHash.length > 0);
  } catch {
    return false;
  }
}

export async function setPassword(username: string, passwordHash: string, email = ''): Promise<void> {
  await connectToDatabase();
...
export async function getPasswordHash(username: string): Promise<string> {
  try {
    await connectToDatabase();
    const found = await UserSettingsModel.findOne({ username }).select('passwordHash').lean();
    return found && typeof found.passwordHash === 'string' ? found.passwordHash : '';
  } catch {
    return '';
  }
}
